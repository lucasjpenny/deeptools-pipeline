...
rule all:
  input:
    expand(
            [path_to_data + '/results/{cohort}/plots/{cohort}.{bed}.{identifier}.matrix_plot.pdf'.format(
                    cohort=v[0],
                    sample=v[1],
                    identifier=config["data"]["cohorts"][v[0]]["settings"]["identifier"],
                    bed = z
                ) for v in get_all_samples_with_cohorts() for z in extract_bedfile_names(config["data"]["bed_dir"])
            ]
        )
...
rule bamCoverage:
  ...
  resources: cpus=2, mem_mb=30000, time_min=400
  run:
...
rule compute_matrix_cohort:
  ...
  output:
    matrix=path_to_data + "/results/{cohort}/matrices/{cohort}.{bed}.{identifier}.matrix.gz"
  params:
    code= lambda wildcards: config['data']['cohorts'][wildcards.cohort]['settings']['computeMatrix_code'],
    bed= lambda wildcards: config['data']["bed_dir"] + "/" + wildcards.bed
  resources: cpus=2, mem_mb=30000, time_min=500
  shell:
    """
...
rule plot:
    input:
        lambda wildcards:
        path_to_data + '/results/' + wildcards.cohort + '/matrices/' + wildcards.cohort + '.' + wildcards.bed + '.' + wildcards.identifier + '.matrix.gz'
