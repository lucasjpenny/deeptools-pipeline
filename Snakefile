...
rule all:
  input:
    expand(
            [path_to_data + '/results/{cohort}/plots/{cohort}_{identifier}.matrix_plot.pdf'.format(
                    cohort=v[0],
                    sample=v[1],
...
rule compute_matrix_cohort:
  ...
  output:
    matrix=path_to_data + "/results/{cohort}/matrices/{cohort}_{identifier}.matrix.gz"
  params:
    code= lambda wildcards: config['data']['cohorts'][wildcards.cohort]['settings']['computeMatrix_code'],
    bed= lambda wildcards: config['data']['cohorts'][wildcards.cohort]['settings']['bed_dir']
  resources: cpus=2, mem_mb=25000, time_min=180
  shell:
    """
...
rule plot:
    input:
        lambda wildcards:
        path_to_data + '/results/' + wildcards.cohort + '/matrices/' + wildcards.cohort + '_' + wildcards.identifier + '.matrix.gz',
    output:
        plot_file = path_to_data + '/results/{cohort}/plots/{cohort}_{identifier}.matrix_plot.pdf'
    params:
